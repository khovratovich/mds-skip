...
\begin{frame}{Upper bounds on the round number skipped}
 Ongoing research: we show that all round skipping attacks reduce to two conditions:
 \begin{itemize}
        \item Existence of a truncated differential trail with few active S-boxes,
        and of a regular differential trail with at least that many S-boxes.
    ...
    \end{itemize}
Parallels to the rebound attack on AES.
...
\begin{remark}
    3-round skipping trails for MDS-based SPN structures are yet to be discovered,
    but we can upper bound the dimension of the solution space already now.
\end{remark}
...
\begin{frame}[c]{}
\begin{center}
\Large{Bounty program and prizes}
\end{center}
\end{frame}


\begin{frame}{Bounty program 2026}
   Bounty program will consist of the following parts:
    \begin{itemize}
        ...
        \item \textsc{Ultimate prize}: Partial Poseidon1 collisions.
        Collide on as many output field elements as possible.
    \end{itemize}

    Concrete instances of the challenges will be published Tuesday March 24th at
    \url{https://poseidon-initiative.info}. They will
    have a short seed to prefix all hash inputs (!).
\end{frame}
...
\end{align}
\item \textbf{Ultimate prize:} Collision on $t$ field elements,
the total prize fund is close to \$1M.
\end{itemize}
